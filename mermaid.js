...
function parseDiagram(source){
  ...
  source.split(/\r?\n/).forEach(raw=>{const line=raw.trim();if(!line||/^(?:flowchart|graph)\s/i.test(line)){return;}if(line==='end'){stack.pop();return;}const group=line.match(/^subgraph\s+(.*)$/i);if(group){const sub={label:unquote(group[1]),members:[]};diagram.subgraphs.push(sub);stack.push(sub);return;}
    ...
    const definition=line.match(/^([A-Za-z_]\w*)\s*(\(\[.*\]\)|\{\{.*\}\}|\(\(.*\)\)|\[.*\]|\(.*\))$/);if(definition){const[shape,label]=shapeFromToken(definition[2]),node=ensure(definition[1],label,shape);node.label=label;node.shape=shape;}
  });return diagram;
}
function layoutDiagram(diagram){
  ...
  const size=node=>{node.width=node.width||Math.min(300,Math.max(132,node.label.length*7+36));node.height=node.height||Math.max(48,Math.ceil((node.label.length*7+36)/300)*18+24);return node;};
  diagram.nodes.forEach(node=>graph.setNode(node.id,size(node)));
  diagram.subgraphs.forEach((group,index)=>{const id=`__subgraph_${index}`;group.id=id;graph.setNode(id,{label:group.label});group.members.forEach(member=>graph.setParent(member,id));});

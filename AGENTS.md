...
- `mermaid.js` is **NOT the Mermaid library** — a hand-written parser/layout/serializer for a small flowchart subset (no `classDef`, styling, or non-flowchart types). Exports `parseDiagram`, `layoutDiagram`, `applyMermaid`, `toMermaid`; CommonJS guard on line 25 enables Node tests. Empty subgraphs never enter the dagre graph (a childless cluster makes dagre return `null` coordinates for the whole layout); they keep their explicit bounds (or a default box) until they gain members. Serializer ceiling: `toMermaid` quotes node and subgraph labels when the naive form would misparse, but edge labels cannot contain `|` and no label can contain a newline.
- `canvas.js` owns rendering + interactions: `render()`, `drawEdges()`, `positionSubgraphs()`, pan/zoom, drag (dropping a node into/out of a subgraph re-lays out), inline edit, context menu.

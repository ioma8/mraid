...
```
vendor/dagre.min.js → state.js → history.js → mermaid.js → canvas.js → ui.js → main.js
```
...
- `main.js` boots the diagram via `relayout()`.
...
| Path | Purpose |
|---|---|
| repo root | Source of truth: `index.html`, `state.js`, `history.js`, `mermaid.js`, `canvas.js`, `ui.js`, `main.js`, `style.css` |
| `vendor/` | Vendored `dagre.min.js` + `DAGRE-LICENSE` (MIT, Chris Pettitt) |
| `native/` | `main.swift` (WKWebView wrapper) + `build.sh` (app/docs build) |
| `tests/` | `mermaid-parser.test.js` + `history.test.js` + `history-coalescing.test.js` + `drag-history.test.js` + `complex-diagram.mmd` fixture |
...
| Action | Command |
|---|---|
| Run tests | `node tests/mermaid-parser.test.js`, `node tests/history.test.js`, `node tests/history-coalescing.test.js`, `node tests/drag-history.test.js` |
...
- **Keyboard shortcuts** (ui.js): `Space`=pan modifier, `N`=add node, `C`=connect mode, `⌘/Ctrl+J`=duplicate, `⌘/Ctrl+Z`=undo, `⌘/Ctrl+⇧Z` or `⌘/Ctrl+Y`=redo, `Delete/Backspace`=delete — all ignored when focus is in an input (so undo inside the code editor / label field stays native text undo).

## Important Files

- `index.html` — app shell; script-load order (lines 48–54) is the wiring contract.
- `state.js` — DOM cache (lines 1–16) + all state (lines 18–25). No persistence exists despite the "Saved locally" status dot.
...
- **Coverage:** none — no tool, no thresholds, no CI enforcement. Four regression tests make up the whole suite.
